...
const Customer = () => {
  ...
  const GetUserAccess = () => {
    const userrole =
      sessionStorage.getItem("userrole") != null
        ...
        : "";
    fetch(
      "http://localhost:3002/roleaccess?role=" + userrole + "&menu=customer"
    )
      .then((response) => {
        if (!response.ok) {
          navigate("/");
  ...
  const handleadd = () => {
    if (haveadd) {
      ...
      toast.warning("You don't have access for add");
    }
  };
  const handleedit = () => {
    if (haveedit) {
      ...
      toast.warning("You don't have access for Edit");
    }
  };
